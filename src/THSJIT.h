...
THS_API JITModuleWrapper * THSJIT_loadModule(const char* filename);

// Gets the number of submodules contained into the source module.
THS_API long THSJIT_getNumModules(const JITModuleWrapper * mwrapper);

// Gets the name of the module contained into the input wrapper at the given index.
THS_API const char* THSJIT_getModuleName(const JITModuleWrapper * mwrapper, const int index);

// Gets the sub-module contained into the input wrapper at the given index.
THS_API JITModuleWrapper * THSJIT_getModuleFromIndex(const JITModuleWrapper * mwrapper, const int index);

// Gets the sub-module contained into the input wrapper with the given name.
THS_API JITModuleWrapper * THSJIT_getModuleFromName(const JITModuleWrapper * mwrapper, const char* name);

// Returns the number of inputs expected by the input module.
THS_API int THSJIT_getNumberOfInputs(const JITModuleWrapper * mwrapper);

// Returns the number of outputs generated by the input module.
THS_API int THSJIT_getNumberOfOutputs(const JITModuleWrapper * mwrapper);

// Returns the type of the nth-input.
THS_API void * THSJIT_getInputType(const JITModuleWrapper * mwrapper, const int n);

// Returns the type of the nth-output.
THS_API void * THSJIT_getOutputType(const JITModuleWrapper * mwrapper, const int n);

// Returns the int8_t code for the input type.
THS_API int8_t THSJIT_typeKind(const JITTypeWrapper * twrapper);

// Returns the int8_t code for the raw type of the tensor.
THS_API int8_t THSJIT_getScalarFromTensorType(const JITTensorTypeWrapper * ttwrapper);

// Gets the number of dimensions of the input tensor type.
THS_API int THSJIT_getTensorTypeDimensions(const JITTensorTypeWrapper * ttwrapper);

// Gets the number of device of the input tensor type.
THS_API const char * THSJIT_getTensorDevice(const JITTensorTypeWrapper * ttwrapper);

// Forward pass over the input module using the input tensor.
THS_API TensorWrapper * THSJIT_forward(const JITModuleWrapper * mwrapper, const TensorWrapper ** twrapper, const int length);

// Disposes the module.
THS_API void THSJIT_moduleDispose(const JITModuleWrapper * mwrapper);

// Disposes the type.
THS_API void THSJIT_typeDispose(const JITTypeWrapper * twrapper);